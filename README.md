...
5. **Dokunuş eşlemesi** (germe / oranlı / çapalı) decides how the measured table becomes screen
   points. `çapalı` is the default and behaves like the engine: one scale near the screen edges
   (so a toolbar button stays square and keeps its gap from the edge) and a continuous middle
   band that absorbs a different aspect ratio. On the aspects the table was measured for it is
   the identity. If every tap misses *in the same direction*, switch it here — no rebuild.
6. **Planı önizle** runs the decomposition only: the log reports how many layers were planned,
   which were dropped for the budget, and the overlay shows the plan — no touches.
7. **Katman sınırı** (the label prints the game's own cap next to your limit, e.g.
